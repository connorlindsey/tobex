defmodule TobexWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-2 text-sm leading-6 text-gray-600">
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          "mt-1 block w-full rounded-lg text-gray-900 focus:ring-0 sm:text-sm sm:leading-6",
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-8 sm:w-full">
        <thead class="text-sm text-left leading-6 text-gray-500">
          <tr>
            <th :for={col <- @col} class="p-0 pr-4 pb-4 font-normal"><%= col[:label] %></th>
        ...
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-gray-50">
            ...
            <td :if={@action != []} class="relative w-14 pr-4">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TobexWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TobexWeb.Gettext, "errors", msg, opts)
    end
  end
